  ...
  switch (event.name) {
    case 'CommitCallback': {
      const offsetCommitted = event.payload.tpl
        .filter((topicPartition) =>
          topicPartition.partitionOffset.find((partitionOffset) => partitionOffset.offset.offset),
        ) // Filter only committed offsets
        .flatMap((p) =>
    ...
    default: {
      console.log(
        'Relalance:',
        event.name,
        event.payload.tpl.map(
          (it) =>
            `Topic: ${it.topic},
                    ${it.partitionOffset.map((po) => `partition: ${po.partition}`).join(',')}`,
        ),
      )
    }
  }
