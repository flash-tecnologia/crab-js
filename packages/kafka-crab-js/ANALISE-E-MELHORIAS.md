...
- [x] Eliminar as fontes de perda silenciosa mapeadas em "Brechas restantes" (P2/P3/P4/H2): mapa substituído por `oneshot` (sem `Pending` para expirar, sem flush global para roubar, sem `Vacant` tardio); drenagem no disconnect com `warn(dropped=N)`.
- [ ] Medir throughput, latência p95/p99, RSS, memória externa e estado após cleanup antes de afirmar ganhos de desempenho.
- [x] **Adotar `tokio::sync::oneshot` por mensagem para as confirmações do producer (M06).** `Sender` no `DeliveryOpaque` (em `Box`); timeout, callbacks tardios, falhas parciais e `autoFlush: false` preservados; `dashmap`/`nanoid` removidos; serialização JS redundante retirada. Contratos provados por 43/43 unit, lifecycle 12/12 e manual-commit 9/9 com broker real; smoke 2000×1 KB sem erros (flush-bound). Sem bench de producer no harness: comparação p95/p99 dedicada segue pendente. Ver `docs/rfc/implemented/0012-oneshot-delivery-tracking/`.
...
| Tracking do producer (`oneshot`)                              | Sem mapa/IDs/nanoid; mesmos 43 + lifecycle/manual com broker real           |
| Bench de producer v4 vs anterior                              | Paridade ~971 op/sec e p95/p99 idênticos (flush-bound); ganho é estrutural  |
| Storm de timeout (300 × 64 KiB, blackhole)                    | Sem crescimento de RSS retido em nenhum dos dois; sem leak para corrigir    |
